...
if __name__ == '__main__':
    ...
    for i in range(13):
        print('GA is now tackling  "' +"F"+str(i+1)+ '"')
        for j in range(n):
            s=GA(getattr(bm, "F"+str(i+1)), lb[i], ub[i], dim, popSize, Iter)
            z+=s.convergence[-1]
            t+=s.executionTime
        z_a.append((z/n))
        t_a.append((t/n))
        ...
        asq = numpy.array(z_a)
        asb = numpy.array(t_a)
        numpy.savetxt("D:\\data\\GAz.csv", asq, fmt='%f', delimiter=",")
        numpy.savetxt("D:\\data\\GAt.csv", asb, fmt='%f', delimiter=",")
