...
#HHO(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter)
...
if __name__ == '__main__':
    ...
    for i in range(13):
        print('HHO is now tackling  "' +"F"+str(i+1)+ '"')
        for j in range(n):
            s=HHO(getattr(bm, "F"+str(i+1)), lb[i], ub[i], dim, popSize, Iter)
            z+=s.best
            t+=s.executionTime
        z_a.append((z/n))
        t_a.append((t/n))
    ...
    asq = numpy.array(z_a)
    asb =numpy.array(t_a)
    numpy.savetxt("D:\\data\\HAz.csv", asq, fmt='%f', delimiter=",")
    numpy.savetxt("D:\\data\\HAt.csv", asb, fmt='%f', delimiter=",")